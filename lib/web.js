...
function normalizeFilename(filename) {
    return filename.trim().replace(/[^a-z0-9._\-]/gi, '_').replace(/_{2,}/g, '_').toLowerCase()
}
    ...
    constructor(server, webSettings, adapter, instanceSettings, app) {
        ...
        if (sharp) {
            if (!fs.existsSync(path.join(root_path, "/userFiles/thumbnail"))) {
                fs.mkdirSync(path.join(root_path, "/userFiles/thumbnail"), { recursive: true });
            }
        }
        ...
        const uploadFilter = function (req, file, cb) {
            const modFile = normalizeFilename(file.originalname);
            // filter rules here
            if (fs.existsSync(path.join(root_path, "/userFiles/", modFile))) {
                cb({ error: `File ${modFile} is already uploaded!`, filename: modFile })
            } else {
                cb(null, true);
            }
        }
        ...
        this.app.post("/" + this.config.route + "upload", (req, res) => {
            upload(req, res, async (err) => {
                ...
                if (err || req.file === undefined) {
                    adapter.log.warn(`[${this.namespace}] Error while uploading! Error: ${err.error}`);
                    response = {
                    ...
                    try {
                        ...
                        adapter.log.warn(`[${this.namespace}] Error while using sharp for thumbnail creation! Error: ${err}`);
                    }
                }
                return res.contentType("application/json").status(200).send(response);
            });
        });
                    ...
                    if (query.image) {
                        ...
                        if (fs.existsSync(imageToDelete)) {
                            fs.unlink(imageToDelete, (err) => {
                                if (err) {
                                    adapter.log.error(`[${this.namespace}] Could not delete the file ${imageToDelete}. Error: ${err}`);
                                    res.contentType("application/json").status(200).send({ error: err, filename: null });
                                } else {
                                    // Delete the thumbnail as well
                                    if (fs.existsSync(path.join(root_path, "/userFiles/thumbnail/", query.image))) {
                                        fs.unlinkSync(path.join(root_path, "/userFiles/thumbnail/", query.image));
                                    }
                                    res.contentType("application/json").status(200).send({ error: null, filename: query.image, msg: 'File successfully deleted!' });
                                }
                            });
                        } else {
                            res.status(200).send(`[${this.namespace}] Image "${query.image}" not found!`);
                        }
                    } else {
                        res.status(404).send(`[${this.namespace}] No image for deletion provided!`);
                    }
