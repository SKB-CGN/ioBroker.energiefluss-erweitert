...
const error_icon =
    '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><rect x="0" y="0" width="24" height="24" fill="transparent" /><path fill="coral" d="M11 15h2v2h-2v-2m0-8h2v6h-2V7m1-5C6.47 2 2 6.5 2 12a10 10 0 0 0 10 10a10 10 0 0 0 10-10A10 10 0 0 0 12 2m0 18a8 8 0 0 1-8-8a8 8 0 0 1 8-8a8 8 0 0 1 8 8a8 8 0 0 1-8 8Z"/></svg>';
...
try {
    sharpModule.sharp = require('sharp');
    sharpModule.enabled = true;
} catch (err) {
    sharpModule.error = err;
}
...
class ProxyEnergieflussErweitert {
    ...
    constructor(server, webSettings, adapter, instanceSettings, app) {
        ...
        if (sharpModule.enabled) {
            adapter.log.info(`[${this.namespace}] Sharp was found and can be used for image processing!`);
        } else {
            adapter.log.error(`[${this.namespace}] Sharp was not found! Image processing will be disabled!`);
        }
        ...
        // Normal requests
        this.app.use(`/${this.config.route}`, async (req, res, next) => {
            ...
            if (req.method == 'GET') {
                ...
                switch (query.serve) {
                    ...
                    case 'instances': {
                        await generateManifest();
                        const sendManifest = generatedManifest.shortcuts;
                        res.status(200).json(sendManifest);
                        break;
                    }
                    ...
                    case 'icon': {
                        if (query.icon) {
                            ...
                            if (Object.prototype.hasOwnProperty.call(iconCacheObject, queryIcon)) {
                                ...
                                res.status(200).json(iconCacheObject[queryIcon]);
                                adapter.log.debug(
